...
public class BushFruits : MonoBehaviour
{
    ...
    public int HarvestFruit()
    {
        if (ready)
        {
            ...
            timer = Time.time + respawnTime[(int)bushVisual.GetVariant()];
            return 2; // Always return 2 fruits regardless of bush type
        }
        else
        {
            return 0;
    ...
    /// <summary>
    /// Returns the fruit variant type of this bush
    /// </summary>
    public BushVariant GetFruitVariant()
    {
        return bushVisual.GetVariant();
    }
